...
func TestXLStorageXlStorageListDir(t *testing.T) {
	...
	if runtime.GOOS != globalWindowsOSName {
		...
		if err = xlStorageNew.DeleteFile(context.Background(), "mybucket", "myobject", false); err != errFileAccessDenied {
	...
	// TestXLStorage for delete on an removed disk.
	// should fail with disk not found.
	err = xlStorageDeletedStorage.DeleteFile(context.Background(), "del-vol", "my-file", false)
	if err != errDiskNotFound {
		t.Errorf("Expected: \"Disk not found\", got \"%s\"", err)
	}
}
...
func TestXLStorageDeleteFile(t *testing.T) {
	...
	for i, testCase := range testCases {
		if err = xlStorage.DeleteFile(context.Background(), testCase.srcVol, testCase.srcPath, false); err != testCase.expectedErr {
...
func TestXLStorageVerifyFile(t *testing.T) {
	...
	if err := xlStorage.DeleteFile(context.Background(), volName, fileName, false); err != nil {
		t.Fatal(err)
	}
