...
func TestHealObjectCorrupted(t *testing.T) {
	...
	err = firstDisk.DeleteFile(context.Background(), bucket, pathJoin(object, xlStorageFormatFile), false)
	if err != nil {
	...
	err = firstDisk.DeleteFile(context.Background(), bucket, pathJoin(object, fi.DataDir, "part.1"), false)
	if err != nil {
	...
	for i := 0; i <= len(er.getDisks())/2; i++ {
		er.getDisks()[i].DeleteFile(context.Background(), bucket, pathJoin(object, xlStorageFormatFile), false)
	}
