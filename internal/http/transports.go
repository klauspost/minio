...
const (
	TransportWriteSize = 32 << 10 // 32KiB moving up from 4KiB default
	TransportReadSize  = 32 << 10 // 32KiB moving up from 4KiB default
)
...
func (s ConnSettings) getDefaultTransport(maxIdleConnsPerHost int) *http.Transport {
	...
	tr := &http.Transport{
		...
		WriteBufferSize:       TransportWriteSize,
		ReadBufferSize:        TransportReadSize,
